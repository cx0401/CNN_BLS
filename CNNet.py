...
class CNN(nn.Module):  # 定义了一个类,名字叫CNN
    ...
    def feature_extract(self, x, i):
        ...
        conv_out = F.max_pool2d(F.relu(self.conv[i](x)), (1, 1))  # 先卷积,再池化
        res = conv_out.view(conv_out.size(0), -1)  # 将conv3_out展开成一维(扁平化)
        return res

    def forward(self, x):  # 定义了forward函数
        # Max pooling over a (2, 2) window
        conv1_out = F.max_pool2d(F.relu(self.conv1(x)), (1, 1))  # 先卷积,再池化
        # If the size is a square you can only specify a single number
        conv2_out = F.max_pool2d(F.relu(self.conv2(conv1_out)), 1)  # 再卷积,再池化
